...
#### Chunk E — vibe + media project intakes

**Vibe intake** (`src/pages/ProjectNewVibe.tsx`): 4 steps.
1. **Idea** — title + one-sentence vision (input + textarea).
2. **Stack sketch** — free-text "what tools might you use?" field (vibe coders often don't pre-spec stacks) + a small chips row of popular vibe tools (Cursor, Claude Code, v0, Lovable, Replit, Supabase) they can click to accept as suggested stack. Maps to `Project.stack`.
3. **Gap topics** — same `gapTopicIds` picker as the dev intake.
4. **Save as plan / Start now** — same status-pick buttons.

**Media intake** (`src/pages/ProjectNewMedia.tsx`): 4 steps.
1. **Concept** — title + one-sentence concept.
2. **Output kind** — radio: image / video / voice / audio / multi. Maps to a new `mediaKind` field on Project (optional string; extend type).
3. **Tools** — chips: Midjourney, Sora, ElevenLabs, Runway, Suno, Flux, DALL-E, Krea, plus a free-text override. Maps to `Project.stack`.
4. **Save as plan / Start now**.

**Dispatcher:** `ProjectNew.tsx` adds branches for `vibe` → `ProjectNewVibe`, `media` → `ProjectNewMedia`. Keep the existing office + build/dev branches.
