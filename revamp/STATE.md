...
- [ ] **Learn page section-homepage** — top of `/learn` becomes a mini-dashboard showing learning progress (overall mastery, per-track progress, recent lesson, next-up) *before* the tracks list. Section-scoped version of the main Dashboard's Learn panel.
- [ ] **Projects page section-homepage** — top of `/projects` becomes a mini-dashboard showing projects progress (status counts, active project resume, recent activity) *before* the projects list. Section-scoped version of the main Dashboard's Projects panel.
- [ ] **Electrified progress bars** — redesign `ui/ProgressBar` (and every surface that uses it: Learn tracks, topic mastery, overall progress, projects) from the current filled-pill style to a thin glowing line that grows left-to-right, echoing the charts from the legacy site. When the bar has milestones, render them as small circular nodes along the line that "light up" as progress reaches them. Keep it subtle — accent-tinted glow only, no heavy bloom. Animation re-triggers on every scroll-back-into-view (IntersectionObserver, not one-shot). Only one motion at a time per element — freeze any gradient drift / hue cycle / pulse to static while the growth animation plays. Respect reduced-motion (no grow animation, no re-trigger, just static lit state).
