...
- **One `--danger` for every destructive action (M9.17c):** all delete / wipe / remove-permanent buttons share a single hue via `--danger` → derived `--danger-{solid,border,glow,surface,ink,hover}`. No per-component hardcoded reds (no `#c0392b`, `#f43f5e`, etc. inside destructive selectors). Tune the hue in one place, every destructive surface retunes. Out of scope: sentiment / status pills where red means *severity*, not *action* — those keep their own reds.
- **No text-selection on tappable surfaces (M9.17d):** every tappable / draggable / clickable element (card rows, action buttons, grab handles, filter chips, nav chrome, CTAs) carries `user-select: none` + `-webkit-user-select: none` + `-webkit-touch-callout: none`. Prevents iOS long-press from popping the selection loupe and keeps drags from leaving highlights behind. Informational-only surfaces (summaries, meta rows, coverage pills, long-form body copy) stay selectable on purpose so users can copy text.
...
- **Soft glows, not neon.** Identity glows via low-alpha `box-shadow` hugging the border; background-tier, never foreground-dominant. Specific components that visibly cut off at their edges (like `.dash-tile:hover`) use a soft blur-gradient halo per-component; other components that don't exhibit the cutoff keep whatever structure they already have — no sweeping rule.
...
## Tile grammar (M9.18 / M9.19 — shipped in v1.0.0)

Every list-item tile site-wide (Learn list · dashboard Learn panel · dashboard Projects panel · future Tools / Snippets) renders through a single `.dash-tile` component. One learned interaction model, one shared geometry.

- **4-quadrant condensed anatomy.** Leading 28px grab column; right of that, three stacked content rows: **row 1** = status cluster (duration chip + state pill), right-aligned. **Row 2** = eyebrow / category ("Course · Academy"). **Row 3** = title (16px, 600 weight — the visual anchor). Optional rows below: flags / coverage / progress. Summary does NOT render in condensed view; it reads in the expanded card only. Layout uses `grid-template-areas` so rows collapse cleanly when not emitted.
- **Pin = "send to Learn," universal.** Pin icon appears only on items **outside** Learn (Tools / Projects / Snippets / News). Tapping pin on a Tools card creates a Learn draft. Once an item is IN Learn, there is **no** pin affordance — it's already here. Inventory rollups ("2 tools · 1 snippet") that used to borrow the pin visual now live in the flag row as read-only pills.
- **State pill = status; CTA = action.** Condensed state pill reads as a status descriptor (`NOT STARTED` / `CONTINUE` / `DONE`). Action verbs (Start lesson → / Continue lesson → / Review lesson → / Open course → / Modify draft →) live only on the expanded card's primary CTA button. Never double-book the two.
- **Mastery collapses into completion.** Done flag is driven by real progress (quiz passed / lesson marked complete), not a user-set toggle. No separate manual-mastery button on Learn tiles. Tools still have mastery (different concept: "I'm proficient with this tool") — distinct surface, distinct meaning.
- **Progressive disclosure on tap.** Tap a condensed tile → it grows into a floating detail card at higher z-index over a dimmed backdrop. Origin animates from the tapped tile's centerpoint via compositor-only `transform: translate + scale(0.2→1)`. Expanded card shows full eyebrow / title / status / summary / external link / inventory, with a single big CTA at the bottom firing the real navigation. Backdrop tap / close-X / Esc dismisses. Reverses to origin on close. Reduced-motion skips the scale, keeps the fade.
- **Drafts swipe-to-reveal delete.** Right-to-left swipe on a draft tile translates it `translateX(-80px)`, revealing a red Delete button behind it (iOS Mail pattern). Tap the red → commit delete. Auto-closes after 4s. Tap-outside closes. Only drafts delete via UI — courses + authored lessons are permanent content.
- **Drag lift goes brighter, not darker.** Per Apple HIG gestures-and-drag §4, picked-up state = scale ~1.04 + shadow increase + brighter surface. `.dash-tile.is-dragging` pushes background to pure `#ffffff`; the default `:active` darken is gated with `:not(.is-dragging)` so the held finger doesn't inherit it.
- **Duration chip always populated on tasks.** Learn items (lessons / courses / drafts / quizzes) always carry a time. When minutes isn't authored yet, the chip reads `? m` — honest placeholder, not a fabricated default. Future authored content will require minutes at authoring time. Projects have no duration concept — slot stays reserved-but-invisible.
- **Unified classes with opt-in overrides, never sweeping cascades.** Prefer one shared class/token that components opt into over applying a rule across many disparate selectors. CSS cascades can shift compositor layers / stacking contexts / backdrop-filter interactions invisibly — a change that looks purely cosmetic can regress activation behavior elsewhere. M9.18a.2 reverted in M9.18b.2 is the canonical lesson.
- **TDZ discipline on the initial-render path.** Any module-level `const` / `let` referenced from a render fn fired by the initial `applyRoute → applyFilter("home") → replayHomeAnimations` at L602 of `js/app.js` MUST be declared above that call site, or the read throws `ReferenceError` on first load and silently aborts the entire IIFE (killing downstream wiring). The `tdz-audit` skill enforces this before every milestone ship.
