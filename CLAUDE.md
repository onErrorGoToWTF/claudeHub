...
- **Soft glows, not neon — always blur-gradient (M9.18a.2).** Identity glows via low-alpha `box-shadow`, background-tier, never foreground-dominant. Every outset color halo MUST use `blur > 0` so it fades to transparent smoothly: `box-shadow: 0 0 <blur>px 0 <accent-glow>`. Hard spread-only rings (`0 0 0 Npx <color>`, blur=0) are **banned as glows** because they cut off sharply at the element edge and at any parent `overflow` clip. Reach guide: blur ≈ 3–6× the visual halo width you want (e.g. `0 0 20px 0 var(--accent-glow)` for a ~16px-reaching tile halo). Carve-outs: inset highlights (`inset 0 1px 0 ...`), sub-2px hairline outlines, and armed-state danger rings that use blur ≥ 8px remain compliant because their edges are soft.
